...
So, neat. You can write AWK scripts and run them using `awk -f script.awk`. They need a `BEGIN` function, which is the `main` function from most other languages. And, yeah, you can even create your own functions and do some basic math. That's starting to seem more like a real language than just a little commnanline utility.
