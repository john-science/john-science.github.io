...
function init_board() {
	...
	for (i=0; i<9; i++) {
		...
		buttons[i].addEventListener("click", function(e) {mark_square(this.id);});
	}
...
function mark_square(nn) {
	...
	if (number_of_moves > 0) {
		if (number_of_moves == 1) {if (player_moves_first) { ai_move(); }}
		else { ai_move(); }
	}
}
...
function end_game_test(z) {
	if (has_won() == player) {
		alert(buttons[z].value + ' won!');  // TODO: You won!  ...You lost. Ah well.
...
function ai_move() {
	...
	if (number_of_moves == 0) {
		...
		if (grid[center] == 0) { x = center;}
		else { x = corners[Math.floor(Math.random() * 4)]; }
	} else {
		// check if we can win
...
function find_win_posi(v) {
	...
	for (i=0; i<24; i+=3) {
		if ((way[i] + way[i+1] + way[i+2]) == 2 * v) {
			if (way[i] == 0) {return posi[i];}
			else if (way[i+1] == 0) {return posi[i+1];}
			else {return posi[i+2];}
		}
	}
	return -1;
}
...
function set_player_label() {
	document.getElementById("you").innerHTML = player_moves_first ? 'X' : 'O';
}
